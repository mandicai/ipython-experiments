function [center_x, center_y, interp_x, interp_y, mean_band, amplitude_array, std_array] = findMeanBand(angles_no_outliers,velocity_no_outliers, perturbation_frames)
...
y_scale = 5;    % In order to use polar system, data in y direction should have similar amplitude as x data, this is factor to adjust
                % When perform test on the tripped data, make sure use the exactly same scale  
intrp_factor = 6;  % Interpolation to make smooth bound result
                   % Only used for finding bound, no need for
...
center_x = (max(interp_x) + min(interp_x))/2; 
center_y = (max(interp_y) + min(interp_y ))/2;
...
for frm_idx = last_idx:perturbation_frames(1) % (numFrames-1800):numFrames, this is for the last 30 seconds of the trial (post-perturbations)
    x0 = interp_x(frm_idx)-center_x;          % last_idx:perturbation_frames(1), this is for the seconds before the first perturbation 
    y0 = interp_y(frm_idx)-center_y;	
...
amplitude_array = zeros(100,1); % Array that stores amplitudes for each slice
std_array = zeros(100,1); % Array that stores stdevs for each slice
