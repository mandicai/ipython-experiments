...
speeds_plus_data1 = [oxygen_speeds1 trials_proc_data1];
speeds_plus_data2 = [oxygen_speeds2 trials_proc_data2];
% Sort the matrix based on the speeds 
% Sort the first part and store in a variable
block_1_sorted = sortrows(speeds_plus_data1(1:5,:),1);
% Sort the second part and store in a variable 
block_2_sorted = sortrows(speeds_plus_data2(1:5,:),1);
...
ylabel('Oxygen Consumption (mL/m/kg)');
% Define the range of the axes
ylim([((min(averages(:,5)) + max(averages(:,5)))/2) - 0.04, ((min(averages(:,5)) + max(averages(:,5)))/2) + 0.04]); % 0.08 for subject 23