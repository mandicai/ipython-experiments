...
% Defining dynamic arrays to place the minima and indices in
xmax = [];
imax = [];
xmin = [];
imin = [];
...
% Vector input?
% numel function returns the number of elements 
Nt = numel(x);
if Nt ~= length(x)
...
% The any function checks to see if the elements are non-zero
% If the values in dx are zero, then it's a horizontal line
if ~any(dx)
 return
end

% Find function find the indices where a particular value occurred 
