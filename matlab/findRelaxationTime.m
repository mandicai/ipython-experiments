...
take_out_fault_perturbs = [2, 5, 13, 14, 17];
...
% for i = take_out_fault_perturbs
    for j = perturbation_data(i):perturbation_data(i+1)
        ...
        if out_of_bounds_array(j) == 0 && out_of_bounds == true % Going from out of bound to in bound
            ...
            if counter_zero > num_frame_stride/6
                relaxation_time(i) = counter_relaxation_time - num_frame_stride/6;
        ...
        if out_of_bounds_array(j) == 1 && counter_zero ~= 0
            ...
            if counter_one > num_frame_stride/6
                counter_zero = 0;
                counter_one = 0;
            end
        end
